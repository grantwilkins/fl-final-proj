...
from backpack_local.utils.unsqueeze import kfacmp_unsqueeze_if_missing_dim
...
def kfac_mat_prod(factors):
    ...
    _, col_dims = zip(*shapes, strict=False)
...
def inv_kfacs(factors, shift=None):
    ...
    shifts = make_shifts()
    return list(starmap(sym_mat_inv, zip(factors, shifts, strict=False)))
...
def kfac_mat_prod_einsum_equation(num_factors):
    ...
    return f"{mat_str}{in_str}->{out_str}"
