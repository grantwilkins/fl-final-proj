"""Dispatch the FMNIST functionality to project.main.
...
from project.task.fmnist_classification.dataset import get_dataloader_generators
from project.task.fmnist_classification.models import get_net
from project.task.fmnist_classification.train_test import get_fed_eval_fn, test, train
...
def dispatch_train(
    ...
    if train_structure is not None and train_structure.upper() == "FMNIST":
...
def dispatch_data(cfg: DictConfig) -> DataStructure | None:
    ...
    # for FMNIST and the dispatch should return None
    ...
    if client_model_and_data is not None and partition_dir is not None:
        ...
        if client_model_and_data.upper() == "FMNIST_CNN":
            return (
                get_net,
                client_dataloader_gen,
                fed_dataloader_gen,
