...
from backpack_local.custom_module.branching import SumModule, _Branch
from backpack_local.custom_module.permute import Permute
from backpack_local.custom_module.reduce_tuple import ReduceTuple
...
def _transform_mul_to_scale_module(module: Module, debug: bool) -> GraphModule:
    ...
    if debug:
        print(
            f"\tMultiplications transformed: {len(nodes_function) + len(nodes_method)}"
        )
...
def _rnn_hyperparams(module: RNN | LSTM) -> tuple[int, int, float, bool]:
...
def _make_rnn_backpack(module: RNN | LSTM) -> Module:
