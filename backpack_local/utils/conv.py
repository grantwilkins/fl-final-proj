...
def unfold_input(module: Conv1d | Conv2d | Conv3d, input: Tensor) -> Tensor:
...
def get_weight_gradient_factors(
    input: Tensor, grad_out: Tensor, module: Conv1d | Conv2d | Conv3d
) -> tuple[Tensor, Tensor]:
...
def unfold_by_conv(input: Tensor, module: Conv1d | Conv2d | Conv3d) -> Tensor:
...
def _grad_input_padding(
    ...
    if len(input_size) != k:
        raise ValueError(
            f"input_size must have {k + 2} elements (got {len(input_size)})"
        )
    ...
    for size, min_size, max_size in zip(input_size, min_sizes, max_sizes, strict=False):
