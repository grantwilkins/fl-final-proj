...
from backpack_local import extensions
from backpack_local.context import CTX
from backpack_local.custom_module.graph_utils import convert_module_to_backpack
from backpack_local.extensions.backprop_extension import BackpropExtension
from backpack_local.utils.hooks import no_op
from backpack_local.utils.module_classification import is_no_op
...
def hook_store_io(
    module: Module, input: tuple[Tensor], output: Tensor | tuple[Tensor]
) -> None:
    ...
    if disable.should_store_io() and is_grad_enabled():
        for i in range(len(input)):
            setattr(module, f"input{i}", input[i])
