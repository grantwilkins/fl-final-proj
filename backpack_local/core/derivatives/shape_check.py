...
def _remove_V_dim(mat):
    if mat.shape[0] != 1:
        raise RuntimeError(
            "Cannot unsqueeze dimension 0. ", f"Got tensor of shape {mat.shape}"
        )
...
def check_shape(mat: Tensor, like: Tensor, diff: int = 1) -> None:
    ...
    if len(mat_shape) - len(like_shape) != diff:
        raise RuntimeError(
            f"Difference in dimension must be {diff}.",
            f" Got {mat_shape} and {like_shape}",
        )
    if mat_shape[diff:] != like_shape:
        raise RuntimeError(
            f"Compared shapes {mat_shape[diff:]} and {like_shape} do not match. ",
            f"Got {mat_shape} and {like_shape}",
        )
...
def check_same_V_dim(mat1, mat2):
    ...
    if V1 != V2:
        raise RuntimeError(f"Number of vectors changed. Got {V1} and {V2}")
